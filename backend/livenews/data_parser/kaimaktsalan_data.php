...
function updateDBSnows($snow_up,$snow_down,$temp) {
    ...
    $query= "UPDATE skicenter SET snow_up=$snow_up,snow_down=$snow_down,temp=$temp WHERE id=4";
    echo "    Query:".$query."\t";
...
function getCurrentDataCondition() {
  ...
  global $reportMessage;
   ...
   global $previous_temp;
    ...
    $ch = curl_init("http://ec2-54-194-95-194.eu-west-1.compute.amazonaws.com/backend2/index.php/skicenter/4");
...
function sendNotificationForLifts() {
  ...
  $body ='{ "channels": ["Kaimaktsalan"],"data": {"alert": "Όλοι οι αναβατήρες στο ΧΚ Καιμακτσαλάν βρίσκονται σε λειτουργία."}}';
...
function sendNotificationForSnow() {
  ...
  $body ='{ "channels": ["Kaimaktsalan"],"data": {"alert": "Φρέσκο χιόνι μόλις έπεσε και σε περιμένει στο Χ.Κ. Καιμακτσαλάν!"}}';
  curl_setopt($ch, CURLOPT_POSTFIELDS, $body);
  curl_setopt($ch, CURLOPT_POST, true);
...
$curl = curl_init('http://www.snowreport.gr/kaimaktsalan/');
...
foreach ( $elements as $element ) {
  ...
  echo $str1;
     ...
     if ((strpos($index,'Μετερίζι') !== false) && ($total_lifts==0)) {
      ...
      updateDBLift("lift_1",$cart["lift_1"],'Μετερίζι',31);


       } else  if ((strpos($index,'Σαραντόβρυση 2') !== false) && ($total_lifts==1)) {
        ...
        updateDBLift("lift_2",$cart["lift_2"],'Σαραντόβρυση 2',32);
            
       } else  if ((strpos($index,'Κρέμαση') !== false) && ($total_lifts==2)) {
        ...
        updateDBLift("lift_3",$cart["lift_3"],'Κρέμαση',33);
            
       } else  if ((strpos($index,'Σαραντόβρυση 1') !== false) && ($total_lifts==3)) {
        ...
        updateDBLift("lift_4",$cart["lift_4"],'Σαραντόβρυση 1',34);
            
       } else  if ((strpos($index,'Baby lift 1') !== false) && ($total_lifts==4)) {
        ...
        updateDBLift("lift_5",$cart["lift_5"],'Baby lift 1',35);
            
       } else  if ((strpos($index,'Baby lift 2') !== false) && ($total_lifts==5)) {
        ...
        updateDBLift("lift_6",$cart["lift_6"],'Baby lift 2',36);
...
$mail->Subject = "Kaimaktsalan Conditions Update :".$date;
